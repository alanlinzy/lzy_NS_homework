...
class myserver(asyncio.Protocol):
    ...
    def write_func(self,message):
        #socket.send()
        print(message)
        self.responsepkt = gamepacket.GameResponsePacket(response=message,
        ...
        for pk in self.deserializer.nextPackets():
            ...
            if pk.DEFINITION_IDENTIFIER == autograder.AutogradeTestStatus.DEFINITION_IDENTIFIER:
                ...
                if pk.submit_status != autograder.AutogradeTestStatus.PASSED:
                    ...
                    print(pk.error)
            elif pk.DEFINITION_IDENTIFIER == gamepacket.GameInitPacket.DEFINITION_IDENTIFIER:
                print("game init")
            ...
            elif pk.DEFINITION_IDENTIFIER == gamepacket.GamePayPacket.DEFINITION_IDENTIFIER:
                self.printpacket(pk)
                print("game response")
                self.startgame()
                ...
                if bhw.example_verify(bank_client, receipt[0], receipt[1], self.account, self.payment, self.unique_id):
                 ...
                 '''
            elif pk.DEFINITION_IDENTIFIER is gamepacket.GameCommandPacket.DEFINITION_IDENTIFIER:
                 print("playing game")
                 if self.game.status == "playing":
                     print(pk.command)
                     output = self.game.command(pk.command)
            else:
                print("what's that")
...
if __name__=="__main__":
    ...
    c = playground.create_server(myserver,'localhost',4236)
