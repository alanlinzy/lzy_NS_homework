...
class TestPoopHandshake(unittest.TestCase):
    ...
    def test_no_error_handshake(self):
        ...
        # Error case 4: Server received two packet with same SYN
        self.server_poop.data_received(HandshakePacket(SYN=1))
        self.server_poop.data_received(HandshakePacket(SYN=1))
        # server still should not be connected
        self.assertEqual(self.server._connection_made_called, 1)
        # there should be 1 blob of bytes from the server for the SYN ACK
        ...
        # Error case 5:  receive wrong packet
        #set server status and recvive wrong data
        self.server_poop.status = "SYN_SENT"
        self.server_poop.data_received(ShutdownPacket().__serialize__())
        self.server_poop.data_received(DataPacket().__serialize__())
        self.assertEqual(self.server._connection_made_called, 1)
        print(self.server.pop_all_data())
        #set server status and recvive wrong data
        self.client_poop.status = "SYN_SENT"
        self.client_poop.data_received(ShutdownPacket().__serialize__())
        self.client_poop.data_received(DataPacket().__serialize__())
        self.assertEqual(self.server._connection_made_called, 1)
        print(self.client.pop_all_data())
